...
def signup_user():
    if request.method == "POST":
        ...
        print(
            f"firsntame: {firstname} lastname: {lastname} email: {email} club: {club}", file=sys.stderr)
...
def update_clubs():
    ...
    interest_response = update_club_interests(
        import_club_interests(spreadsheet, worksheet))
